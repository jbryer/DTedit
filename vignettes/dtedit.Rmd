...
Changes in `dtedit`'s copy of the data as the result of editing within `dtedit` do **not** automatically change the original dataframe **mydata**. The example below has set up an `observeEvent` to detect changes in **Grocery_List_Results$thedata**, which then changes **mydata**. Note that **Grocery_List_Results$thedata** does *not* have trailing parentheses '()'.
...
`DTedit` uses `DT`. `DT` [provides *helper* functions `format*()`](https://rstudio.github.io/DT/functions.html)
...
`datatable.call` as pre-processing function.

The formatting demonstrated below is not actually appropriate to the `mtcars` data,
but is used simply for demonstration purposes. Unfortunately, the formatting does
not work [unless `datatable.rownames` is set to `TRUE`](https://stackoverflow.com/questions/56879672/how-to-replacedata-in-dt-rendered-in-r-shiny-using-the-datatable-function),
...
`dtedit` uses the [`DT`](https://rstudio.github.io/DT/) package to create datatables. [`DT`](https://rstudio.github.io/DT/) is based on [`datatables`](https://datatables.net/). `DT` has an `options` argument. `options` can be passed through to `DT` with `dtedit`'s `datatables.options` argument. Any argument that is not recognized by `dtedit` is passed through to `DT`.

The following example adds [copy, CSV and PDF buttons](https://rstudio.github.io/DT/003-tabletools-buttons.html) to `dtedit`'s datatable by defining `datatable.options` and `extensions` (extensions is not a recognized argument of `dtedit`).
