...
Drop down choices can be defined statically by defining `input.types` as `selectInput`. `selectInput` will look for possible input choices by checking if `input.choices` is defined (as in the example below), the column has defined `factor`s or the pre-existing values already present in the column.
...
`selectInputReactive` is demonstrated in `dtedit_selectInputReactive_demo(display.mode = "showcase")`, with the source code in the `example_selectInputReactive.R` tab. In this demonstration, **names.Types** (user types) and **names.Likes** (potential user Likes) are reactive variables which are used to determine the choices to `selectInputReactive` and `selectInputMultipleReactive` in the **names** DataTable. **names.Types** and **names.Likes** are themselves altered by changes in two other `DTedit` datatables.

The same demonstration is shown for modular `dteditmod` in `dteditmod_demo(display.mode = "showcase")`, with source code in the `app_mod.R` tab. The interacting datatables are in the `Emails` tab.
...
A demonstration of action buttons (two columns in the same datatable) can be seen, with source code, with `dteditmod_fileInput_demo(display.mode = "showcase")`. The code is in the `example_mod_fileInput.R` tab. This demonstration uses the modular `dteditmod`. It also demonstrates use of binary blob data-types, use of `view.cols` and `edit.cols` and the (implicit) use of `fileInput` to store pictures and spreadsheets in the dataframe.
