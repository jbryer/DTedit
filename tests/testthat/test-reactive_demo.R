...
context("reactive_demo") # note that context is not encouraged for testthat >= 2.1
testthat::test_that("reactive_demo", {
  ...
  results <- withr::with_libpaths(tmp_lib, {
    shinytest::testApp(appDir = testthat::test_path("../reactive_demo"), compareImages = FALSE)
  }, action = "prefix")
