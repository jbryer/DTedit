...
context("callbacks_actionButtons") # note that context is not encouraged for testthat >= 2.1
testthat::test_that("callback", {
  ...
  results <- withr::with_libpaths(tmp_lib, {
    shinytest::testApp(appDir = testthat::test_path("../callbacks_actionButtons"), compareImages = FALSE)
  }, action = "prefix")
