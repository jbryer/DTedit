...
context("reactive") # note that context is not encouraged for testthat >= 2.1
testthat::test_that("reactive", {
  ...
  results <- withr::with_libpaths(tmp_lib, {
    shinytest::testApp(appDir = testthat::test_path("../reactive"), compareImages = FALSE)
  }, action = "prefix")
