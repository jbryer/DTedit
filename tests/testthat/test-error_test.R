...
context("error_test") # note that context is not encouraged for testthat >= 2.1
testthat::test_that("error_test", {
  ...
  results <- withr::with_libpaths(tmp_lib, {
    shinytest::testApp(appDir = testthat::test_path("../error_test"), compareImages = FALSE)
  }, action = "prefix")
