...
context("password") # note that context is not encouraged for testthat >= 2.1
testthat::test_that("password", {
  ...
  results <- withr::with_libpaths(tmp_lib, {
    shinytest::testApp(appDir = testthat::test_path("../password"), compareImages = FALSE)
  }, action = "prefix")
