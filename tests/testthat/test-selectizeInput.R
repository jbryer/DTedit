...
context("selectizeInput") # note that context is not encouraged for testthat >= 2.1
testthat::test_that("selectizeInput", {
  ...
  results <- withr::with_libpaths(tmp_lib, {
    shinytest::testApp(appDir = testthat::test_path("../selectizeInput"),
                       compareImages = FALSE)
  }, action = "prefix")
